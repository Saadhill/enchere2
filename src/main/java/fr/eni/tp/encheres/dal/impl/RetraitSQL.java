...
public class RetraitSQL implements RetraitDAO {
   ...
   /* @Override
    public Retrait getRetraitByArticle(ArticleVendu articleVendu) {
        String sql = "SELECT * FROM retrait WHERE id_article = :id_article";
        MapSqlParameterSource namedParameters = new MapSqlParameterSource();
        namedParameters.addValue("id_article", articleVendu);
        Retrait retrait = namedParameterJdbcTemplate.query(sql, namedParameters);
        return retrait;
